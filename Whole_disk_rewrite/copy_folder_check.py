...
for i in tqdm.tqdm(range(len(source_res))):
    ...
    if os.path.exists(source_res[i].replace(source, dest)):
        if os.path.getsize(source_res[i]) == os.path.getsize(source_res[i].replace(source, dest)):
            skip_count += 1
            # print(f"{datetime.datetime.now().strftime('%H:%M:%S')} : skipping {skip_count}/{len(source_res)}: {source_res[i]}")
            pbar.update(math.floor(os.path.getsize(source_res[i]) / 1000000))
            continue
        else:
            os.remove(source_res[i].replace(source, dest))
    pbar.set_description("Copying: " + source_res[i] + "  " + str(math.floor(os.path.getsize(source_res[i]) / 1000000)) + "mb")
...
for i in tqdm.tqdm(range(len(source_res))):
    if os.path.exists(source_res[i].replace(source, dest)):
        if not os.path.getsize(source_res[i]) == os.path.getsize(source_res[i].replace(source, dest)):
